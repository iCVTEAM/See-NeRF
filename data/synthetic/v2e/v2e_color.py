...
def main():
    ...
    # DVS pixel color_mask
    color_masks = np.zeros((400, 400, 3), dtype=np.float)
    color_masks[0::2, 0::2, 0] = 1  # r
    color_masks[0::2, 1::2, 1] = 1  # g
    color_masks[1::2, 0::2, 1] = 1  # g
    color_masks[1::2, 1::2, 2] = 1  # b
    color_masks = color_masks.reshape((-1, 3))
        ...
        with TemporaryDirectory() as source_frames_dir:
            ...
            for inputFrameIndex in tqdm(
                ...
                if inputChannels == 3:  # color
                    ...
                    inputVideoFrame = inputVideoFrame.reshape((-1, 3)) * color_masks
                    inputVideoFrame = np.sum(inputVideoFrame, axis=-1).reshape(400,400)
                ...
                if auto_timestamp_resolution or slowdown_factor != NO_SLOWDOWN:
                    ...
                    for frame_idx, src_file_path in tqdm(
                        ...
                        tgt_file_path = os.path.join(
                            interpFramesFolder, str(frame_idx) + ".npy")
                        interpFramesFilenames.append(tgt_file_path)
                        n += 1
                        np.save(tgt_file_path, src_frame)
