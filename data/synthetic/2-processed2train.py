...
bin_num = 4     # For synthetic data we set b = 4

def events_info_reader(file_dir):
    with open(file_dir, 'r') as file:
        lines = file.readlines()
        events_num = len(lines) - 6
        exposure_start = float(lines[0 + 6].split()[0]) * 1e19
        exposure_end = float(lines[-1].split()[0]) * 1e19
        file.close()
...
def load_events_txt(basedir, views_num, bin_num):
    ...
    # Pre-definition
    event_map = np.zeros((views_num, bin_num, 400, 400), dtype=np.float64) # The final event_map
    ...
    for view_num in range(views_num):
        ...
        file_dir = basedir + '/{0:03d}/v2e-dvs-events.txt'.format((view_num))  # .txt format events file
        ...
        event_t_pre = np.zeros((bin_num, 400, 400), dtype=np.int64)  # offset flag of the first event
        event_t_end = np.zeros((bin_num, 400, 400), dtype=np.int64)  # offset flag of the last flag
        event_t_pre_polar = np.zeros((bin_num, 400, 400))
        event_t_end_polar = np.zeros((bin_num, 400, 400))

        # Record the first splitting time points
        frames_time.insert(0, exposure_start)

        file = open(file_dir, "r")
        # ignore the first 6 info lines
        for i in range(6):
            file.readline()
        ...
        for i in range(bin_num - 1):
            ...
            offset = np.zeros((400, 400), dtype=np.float64)
...
print("------Stage 3: Event Preprocessing------")
...
print("------Stage 4: Images_Pose_Estimation, Image, GT Folders Generation------")
...
for scene in scenes:
    ...
    for i in range(18):
        ...
        blurry_image = torch.tensor(cv2.imread("./{0}/train_blurry/{1:03d}_t_2.png".format(scene, i)), dtype=torch.float)

        # EDI Model
        event_sum = torch.zeros(400, 400)
        EDI = torch.ones(400, 400)
        ...
        cv2.imwrite("./{0}/images_pose_estimation/{1:03d}.png".format(scene, i * (bin_num + 1)), sharp_image.numpy())

        offset = torch.zeros(400, 400)
        for j in range(bin_num):
            offset = offset + events[i][j]
            img = sharp_image * torch.exp(0.3 * torch.stack([offset, offset, offset], axis=-1))
            cv2.imwrite("./{0}/images_pose_estimation/{1:03d}.png".format(scene, i * (bin_num + 1) + 1 + j), img.numpy())

    # Move the testing image for testing pose estimation
    for j in range(17):
        shutil.copy("./{0}/test_ldr/{1:03d}_l_2.png".format(scene, j),
                    "./{0}/images_pose_estimation/{1:03d}.png".format(scene, 18 * (bin_num + 1) + j))

    # Generate Image Folder for training
    os.makedirs("./{0}/images/".format(scene), exist_ok=True)
    for i in range(18):
        shutil.copy("./{0}/train_blurry/{1:03d}_t_2.png".format(scene, i), "./{0}/images/{1:03d}.png".format(scene, i))

    # Generate GT Folder for testing
    if os.path.exists("./{0}/GT/ldr/".format(scene)):
        shutil.rmtree("./{0}/GT/ldr/".format(scene))
    shutil.copytree('./{0}/test_ldr/'.format(scene), "./{0}/GT/ldr/".format(scene))

    # Generate the exposure time of the test LDR images
    np.savetxt("./{0}/exp_times_test.npy".format(scene), np.array([0.001, 0.004, 0.016, 0.064, 0.256]))

    if os.path.exists("./{0}/GT/hdr/".format(scene)):
        shutil.rmtree("./{0}/GT/hdr/".format(scene))
    shutil.copytree('./{0}/test_hdr/'.format(scene), "./{0}/GT/hdr/".format(scene))



print("------Stage 5: Colmap Calling and pose_bounds.npy Generation------")
