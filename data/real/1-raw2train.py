...
scenes = ["bear", "computer", "computerhard", "desktop", "table"]
views_num = 16  # number of training views
bin_num = 4  # value of b
...
for scene in scenes:
    ...
    event_map, frames_weights = load_events_txt("./{0}/events_t2/".format(scene), views_num, bin_num)
...
for scene in scenes:
    ...
    for i in range(views_num):
        ...
        blurry_image = torch.tensor(cv2.imread("./{0}/images_blurry_t2/{1:03d}.jpg".format(scene, i)), dtype=torch.float)
    ...
    for j in range(28):
        shutil.copy("./{0}/images_gt_ldr/{1:03d}_t_2.jpg".format(scene, j),
    ...
    for i in range(views_num):
        shutil.copy("./{0}/images_blurry_t2/{1:03d}.jpg".format(scene, i), "./{0}/images/{1:03d}.jpg".format(scene, i))



# Generate GT Folders for testing
print("------Stage 3: GT Folders Generation------")

for scene in scenes:
    # Generate GT Folder for testing
    os.makedirs("./{0}/GT/".format(scene), exist_ok=True)

    # Move the LDR GT images to GT folder
    if os.path.exists("./{0}/GT/ldr/".format(scene)):
        shutil.rmtree("./{0}/GT/ldr/".format(scene))
    shutil.copytree("./{0}/images_gt_ldr/".format(scene), "./{0}/GT/ldr/".format(scene))

    # Generate HDR GT images
    os.makedirs("./{0}/GT/hdr/".format(scene), exist_ok=True)
    times = np.loadtxt("./{0}/exp_times_test.npy".format(scene), dtype=np.float32)
    print("Scene {0}: Exppsure Times of t0~t4 ".format(scene), times)
    for j in range(28):
        calibrateDebevec = cv2.createCalibrateDebevec()
        imgs = []
        for i in range(5):
            img = cv2.imread("./{0}/images_gt_ldr/{2:03d}_t_{1}.jpg".format(scene, i, j))
            imgs.append(img)

        # Using the Debevec algorithm to merge 5 ldr GT images with exposure times t0-t4 to hdr GT image for each view
        responseDebevec = calibrateDebevec.process(imgs, times)
        mergeDebevec = cv2.createMergeDebevec()
        hdrDebevec = mergeDebevec.process(imgs, times, responseDebevec)
        cv2.imwrite("./{0}/GT/hdr/{1:03d}.hdr".format(scene, j), hdrDebevec)
