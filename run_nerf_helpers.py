...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        self.exps_linears_evs = nn.ModuleList([nn.Linear(1, W // 2)])
        self.evs_l_linner = nn.Linear(W // 2, 1)
    ...
    def crf_evs(self, gray_h):
        for i, l in enumerate(self.exps_linears_evs):
            gray_h = self.exps_linears_evs[i](gray_h)
            gray_h = F.relu(gray_h)
        gray_l = self.evs_l_linner(gray_h)

        gray_l = torch.sigmoid(gray_l)
...
def warm_crf_evs(model, pretrain_iters=1000, device='cuda'):
    params_to_train = [
        {'params': model.exps_linears_evs.parameters()},
        {'params': model.evs_l_linner.parameters()},
    ]
    optimizer_crf = torch.optim.Adam(params_to_train, lr=5e-4)
    ...
    for i in range(pretrain_iters):
        ...
        for i, l in enumerate(model.exps_linears_evs):
            ln_evs = model.exps_linears_evs[i](ln_evs)
            ln_evs = F.relu(ln_evs)
        evs_l = model.evs_l_linner(ln_evs)

        evs_l = torch.sigmoid(evs_l)

        gt_evs = torch.clip(log_crf(torch.exp(ln_x)), 0, 1)

        loss = (evs_l - gt_evs).norm(dim=1).mean()
        print(f"step {i} pre-train loss: {loss.item()}")
        loss.backward()
        optimizer_crf.step()
    return model
...
def point_constraint_synthetic(model):
    ln_x = -torch.ones([3, 1])
    gt = (math.exp(-1) / (math.exp(-1) + 1)) ** (1 / 2.2)
...
def draw_CRF(basedir, model):
    ...
    plt.xlabel("lnE * $\Delta$t", fontsize=24)
    plt.ylabel("Output Value", fontsize=24)
    plt.xlabel("ln(E$\Delta$t)")
...
def draw_CRF_evs(basedir, model):

    ln_x = torch.linspace(-5, 3, 1000).reshape([-1, 1])
    gray_h = ln_x

    for i, l in enumerate(model.exps_linears_evs):
        gray_h = model.exps_linears_evs[i](gray_h)
        gray_h = F.relu(gray_h)
    gray_l = model.evs_l_linner(gray_h)

    gray_l = torch.sigmoid(gray_l)

    x = ln_x.cpu().numpy()
    y = gray_l.detach().cpu().numpy()
    ...
    plt.xlabel("ln(E$\Delta$t)")
    plt.ylabel("Output Value")
    plt.plot(x, y[:, 0:1], color='y', label='CRF evs')
    # plt.plot(x, z_simple, color='y', label='CRF GT')
    plt.legend(['CRF evs', 'CRF GT'])
    plt.grid()
    plt.savefig(os.path.join(basedir, 'CRF_evs.png'))
    plt.close()
