...
```
The data include three folders ("images", "images_pose_etimation", "GT") and four files (events.pt, frames_weights.npy, poses_bounds.npy, exp_times_test.npy) in each scene's folder.
...
## 4.3 Real World Challenge Data
We provide the **Raw Data** and the **Train & Test Data** of the Real World Challenge Datasets. Please down load them [here](https://drive.google.com/drive/folders/13BHOSljQDj8XHH5pY57-LEy3901n9Ix1?usp=drive_link).

### 4.3.1 Raw Data
We directly used the **Raw Data** from the Real-World-Challenge dataset in [E<sup>3</sup>NeRF](https://github.com/iCVTEAM/E3NeRF), which includes the handheld DAVIS 346 captured blurry images and corresponding raw events (**.txt format**) and tripod-fixed DAVIS 346 captured sharp ground truth images. This data is provided to facilitate the future related research. If you simply want train and test our See-NeRF, please refer to the **Train & Test Data**.

### 4.3.2 Train & Test Data
The **Train & Test Data** can be generated from the **Raw Data**. If you want generate the data by yourself, please put the **Raw Data** scene folders into ./data/real_world_challenge/ file and use the command below with [COLMAP](https://colmap.github.io/install.html) installed in your environment. You can also train and test our See-NeRF directly with the provided **Train & Test Data**.
```
python ./data/real_world_challenge/1-raw2train.py
```
The data include three folders ("images", "images_pose_etimation", "GT") and four files (events.pt, frames_weights.npy, poses_bounds.npy, exp_times_test.npy) in each scene's folder.

**"images":** There are 16 blurry images for training in this folder.

**"images_pose_estimation":** There are 140 (16 training views * 7 in each view) LDR pre-deblurred images with EDI model and 28 GT sharp LDR test images in this folder for the pose estimation using [COLMAP](https://colmap.github.io/install.html). Although *1-raw2train.py* include calling COLMAP for pose estimation , we recommand using its GUI Automatic Reconstruction manually with **Shared Intrinsics option activated**.

**"GT":** This folder contains 28 testing views of sharp images.
...
**"exp_times_test.npy":** The specific values of exposure time of the input images.
